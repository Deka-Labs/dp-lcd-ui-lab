 ...
 *
 * @code{.cpp}
 * lcd_ili93xx_fill_area_color(&lcd_driver, 0, 0, 240 - 1, 320 - 1,
 * LCD_ILI93XX_COLOR_GREEN);
 * ...
 * @endcode
 *
 */
typedef struct {
   ...
   * @return zero on success, otherwise non-zero value
   */
  int (*write_words)(void *user_data, uint16_t address, uint16_t *data,
                     size_t size);
 ...
 * @return zero on success, otherwise non-zero value
 */
int lcd_ili93xx_fill_area_color(lcd_ili93xx_driver_t *driver, int16_t x1,
 ...
 * @return zero on success, otherwise non-zero value
 */
int lcd_ili93xx_fill_area(lcd_ili93xx_driver_t *driver, int16_t x1, int16_t y1,
                          int16_t x2, int16_t y2, uint16_t *colors_data);
 ...
 * @return RGB565 color
 */
inline uint16_t lcd_ili93xx_rgb565_convert(uint8_t red, uint8_t green,
                                           uint8_t blue) {
